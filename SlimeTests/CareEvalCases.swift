...
nonisolated enum CareEvalCases {
    ...
    private static func said(_ text: String, daysAgo: Int, hour: Int, about: [Int],
                             stillShowing: Bool = true) -> PastCare {
        ...
        return PastCare(text: text, stillShowing: stillShowing, saidAt: at, about: about.map(day))
    }
    ...
    /// #37~#40 用的那条**已经撤下**的旧话：5 天前晚上说的，针对 7/6/5 那三天，
    /// 挂满 3 天后在 2 天前自动退场。
    ///
    /// 这是 v2 里唯一没被 eval 覆盖过的状态。手工实测（4 次采样）发现
    /// **模型有一半概率以为它还挂在用户眼前** —— pattern 里写「当前悬挂的关心」
    /// 「保持原关心即可」「值得替换」。原因是 prompt 里讲「关怀挂着时怎么判」的
    /// 那三十行是无条件文本，对抗它的只有 payload 里一个「状态」字段。
    private static let retiredHardTimes = said(hardTimes, daysAgo: 5, hour: 20,
                                               about: [7, 6, 5], stillShowing: false)
    ...
    static let all: [EvalCase] = [
        ...
        // ────────────── 保持专项：看起来像该换，其实旧话还接得住 ──────────────
        //
        // 「保持」原本只有 4 条，而且全都很容易看出没变化（内容空洞、明摆着的延续）。
        // 模型天然倾向于说点新的，所以真正该压的是**看起来变了、其实没变**这一类。
        // 每条都标明跟哪条成对、分界线在哪 —— 并排读得通，标注才立得住。

        EvalCase(id: 30, name: "情绪标签变了，但压着的是同一件事",
                 days: [(5, .tired,   "连着加班"),
                        (4, .tired,   "还是加班"),
                        (3, .tired,   "周末也在改"),
                        (2, .tired,   "改到半夜"),
                        (1, .anxious, "后天就要交了，还差一截")],
                 recentlySaid: [saidTired],
                 expected: false,
                 rationale: "最后一天从 tired 变成 anxious，看着像变了，但还在同一个项目里，"
                          + "旧话「别硬撑着」接得住。"
                          + "【分界线：跑出原来那件事了吗】"
                          + "#7「是不是不适合干这行」、#8「做这些到底图什么」已经跑出去了 —— "
                          + "从一个项目变成对自己、对人生的怀疑；#24 换了一整个来源（工作→家里）。"
                          + "这里的焦虑还在项目里面，只是情绪的标签换了。"
                          + "【09-22 定】原本标反了：当时的分界写成「事情变没变」，"
                          + "模型 5/5 判换而且理由站得住 —— 改的不是标注，是分界线的说法"),

        EvalCase(id: 31, name: "新的一天很具体，但没有新角度",
                 days: [(5, .sad,   "方案被打回，说方向不对"),
                        (4, .tired, "改到半夜"),
                        (3, .tired, "还在改"),
                        (2, .tired, "又改了一版"),
                        (1, .sad,   "今天再交上去，评审还是那几句话，一字没变")],
                 recentlySaid: [saidTired],
                 expected: false,
                 rationale: "【和 #20 成对，但难得多】#20 那三天内容空洞（「又是加班」「还是加班」），"
                          + "一眼看得出没新东西；这里最后一天有画面、有细节（「一字没变」），"
                          + "很容易被读成新进展 —— 但**写得更具体不等于跑出去了**，还是这个方案、这件事。"
                          + "【和 #23 的分界】看强度有没有上升：#23 明显加重，这里是平移。"
                          + "【09-22 定】同 #30，原来的分界说法不够准"),

        EvalCase(id: 32, name: "中间回暖了一天，又落回去",
                 days: [(5, .sad,   "很低落，什么都不想做"),
                        (4, .tired, "撑着上班"),
                        (3, .happy, "朋友突然来找我吃饭，笑了一晚上"),
                        (2, .tired, "回到原来的样子"),
                        (1, .sad,   "还是提不起劲")],
                 recentlySaid: [saidTired],
                 expected: false,
                 rationale: "中间那天确实回暖，但不是转折 —— 后面两天又落回去了，旧话仍然贴切。"
                          + "【和 #21 的分界】看**位置和性质**：#21 的 happy 在最后一天，"
                          + "而且是困扰本身解决了（「过了！」）；这里的 happy 在中间、和困扰无关，"
                          + "后面还被推翻。防「见到一天 happy 就宣布转折」"),

        EvalCase(id: 33, name: "最后一天是好事，但跟困扰无关",
                 days: [(5, .tired, "连着加班"),
                        (4, .tired, "还是加班"),
                        (3, .tired, "改到半夜"),
                        (2, .tired, "周末也没歇"),
                        (1, .happy, "楼下面包店买到了最后一个可颂，开心")],
                 recentlySaid: [saidTired],
                 expected: false,
                 rationale: "最后一天是 happy，但那是件小确幸 —— 加班还在继续，旧话没过时。"
                          + "【和 #21 的分界】#21 的好消息**就是困扰本身解决了**（跑出去了）；"
                          + "这里困扰照旧，好消息是另一码事。"
                          + "【和 #32 的分界】#32 考的是「回暖被推翻」，这条考的是「回暖跟困扰无关」。"
                          + "两条一起防「只看最后一天的情绪标签」"),

        // ────────────── 替换专项：困扰结束了，人却空了 ──────────────

        EvalCase(id: 34, name: "旧话针对的事结束了，但人反而空下来",
                 days: [(5, .tired, "连着加班"),
                        (4, .tired, "还是加班"),
                        (3, .tired, "终于交了，累趴在沙发上"),
                        (2, .calm,  "项目结束，突然闲下来"),
                 ...
                 recentlySaid: [saidTired],
                 expected: true,
                 rationale: "旧话「别硬撑着」针对的是加班，而加班已经结束 —— 那句现在读着过时了。"
                          + "但人并没有好起来，只是难受的点换了（从「撑不住」变成「空了」）。"
                          + "【和 #21 的分界】#21 是困扰解决、人也好了 → 换成呼应轻松的话；"
                          + "这里是困扰解决、人反而落下来 → 换成另一种关心。"
                          + "【和 #30 的分界】#30 那件事还在继续，这里那件事真的结束了"),

        // ────────────── 诊断用例：二元契约表达不了的第四种情况 ──────────────
        //
        // shouldShow 是个 Bool，只能表达三种组合：
        //   旧话还贴切 + 没新话  → false（保持，正确）
        //   旧话不贴切 + 有新话  → true （替换，正确）
        //   旧话还贴切 + 有新话  → true （换成更好的）
        //   **旧话不贴切 + 没新话 → 表达不了**
        //
        // 下面两条就是第四种。**标注写 false 是权宜** —— 真答案是「撤掉但不换新的」，
        // 现在没法表达，只能让过时的话挂到满 3 天自动退场。
        //
        // 它们的用途是**诊断**，不是考核：
        //   模型判保持（○）→ 缺口代价小，一句温和但过时的话多挂两天，可以忍
        //   模型判替换（●）→ 看它说了什么。如果憋出一句正确但没用的空话，
        //                    说明它被二元契约逼着说话，该上三元契约（keep/replace/retire）了
        //
        // 等哪天真上了三元契约，这两条的 expected 直接改成 retire，现成的用例。

        EvalCase(id: 35, name: "旧话过时了，但也没什么新话可说",
                 days: [(5, .tired, "连着加班，累"),
                        (4, .tired, "还是加班"),
                        (3, .tired, "撑着做完了"),
                        (2, .calm,  "这周总算没那么赶了"),
                 ...
                 recentlySaid: [saidTired],
                 expected: false,
                 rationale: "【诊断用例，真答案是 retire】旧话「别硬撑着」针对的是前三天的加班，"
                          + "而人已经不硬撑了 —— 那句话现在读着是过时的。"
                          + "但缓和得很平淡，没有任何值得单独说一句的东西，硬说就是空话。"
                          + "【和 #21 的分界】#21 的好转带着事件（「过了！和朋友吃了顿好的」），有话可说；"
                          + "这里的好转是慢慢消退的，没有落点。"
                          + "看模型会不会为了说而说"),

        EvalCase(id: 36, name: "旧话针对的事结束了，新的日子太薄",
                 days: [(5, .tired, "连着加班，累"),
                        (4, .tired, "还是加班"),
                        (3, .tired, "撑着做完了"),
                        (1, .calm,  "没什么特别的")],
                 recentlySaid: [saidTired],
                 expected: false,
                 rationale: "【诊断用例，真答案是 retire】"
                          + "【和 #34 正好成对】#34 也是「加班结束了」，但人空落落的 —— "
                          + "有新的难受，所以该换；这里人没什么感觉，无处可换。"
                          + "同一个前提，一个有落点一个没有。"
                          + "【和 #22 的区别，很微妙但要紧】#22 的困扰还在（项目卡着没解决），"
                          + "旧话仍然贴切，那是**正确的保持**；这里困扰已经结束，"
                          + "旧话不再贴切，是**憋屈的保持**。"
                          + "两条在 Bool 契约里长得一模一样，实际是两回事 —— 这就是盲区"),

        // ────────────── 退场专项：说过、但那句话已经撤下了 ──────────────
        //
        // 第三种 recentlySaid 状态。前面 36 条只有「空」和「还挂着」两种，
        // **这一种一条都没有** —— 而按 CareGate 的推导，每次自然退场 + 冷却之后都会走一次。
        //
        // 两个方向的错都要防，它们来自同一个病根（模型分不清此刻有没有话挂着）：
        //   以为还挂着 → 用替换规则判「保持原关心即可」→ 该说却不说（#38 #40）
        //   忘了说过  → 把已回应过的旧证据重新当成开口理由 → 换个说法重说（#37 #39）

        EvalCase(id: 37, name: "退场后·同一件事还在继续",
                 days: [(7, .tired,   "连着加班，回家倒头就睡"),
                        (6, .tired,   "还是在改这版方案"),
                        (5, .tired,   "改到半夜，眼睛发胀"),
                        (3, .tired,   "周末也在赶，没出门"),
                        (1, .anxious, "还是这个项目，看不到头")],
                 recentlySaid: [retiredHardTimes],
                 expected: false,
                 rationale: "同一件事在继续，没有新角度 —— 旧话说的就是这个观察。"
                          + "宁可不说，也不要换个说法重说一遍。"
                          + "【这条编码了一个产品选择】长期低谷里母鸡的性格："
                          + "标 false = 绝不啰嗦，代价是一个人连着难受两周只听到过一句话。"
                          + "标 true 的话就是承认「时间过去了」本身是理由，但那会滑向定期问候。"
                          + "09-23 定为 false，跟不说清单一致。"
                          + "【和 #39 的分界】#39 新的那天很薄、旧证据还占着窗口；"
                          + "这里新的几天有实质内容，只是主题相同 —— 考的是「重复观察」"),

        EvalCase(id: 38, name: "退场后·明显加重了",
                 days: [(7, .tired, "连着加班"),
                        (6, .tired, "还是在改这版"),
                        (5, .tired, "改到半夜"),
                        (3, .sad,   "撑不住了，半夜一个人在楼道里坐了很久"),
                        (1, .sad,   "早上起不来，跟公司请了假")],
                 recentlySaid: [retiredHardTimes],
                 expected: true,
                 rationale: "【这条最要紧】旧话针对的是「硬撑」，而现在已经撑不住了 —— "
                          + "从疲惫掉到了停摆，是明显加重，该重新接住。"
                          + "手工实测里模型有一半概率答「保持原关心即可」——"
                          + "可那句话两天前就撤下了，屏幕上什么都没有。"
                          + "**那个错误框架在这个场景里就是该说却不说。**"
                          + "【和 #37 的分界】#37 是平移，这里是下沉"),

        EvalCase(id: 39, name: "退场后·旧证据还在窗口里，新的一天很薄",
                 days: [(7, .tired, "连着加班，累"),
                        (6, .sad,   "方案被否了，说方向不对"),
                        (5, .tired, "改到半夜"),
                        (1, .calm,  "普通的一天，把手边的事做完了")],
                 recentlySaid: [retiredHardTimes],
                 expected: false,
                 rationale: "闸门放行了（有新蛋、天数够、冷却过），但新的那天什么都没有。"
                          + "**陷阱在于 7/6/5 那三天还在 14 天窗口里** —— 关怀退场不会把它们挪走，"
                          + "所以首次开口那条「同一种压力在至少三个日期出现且未缓解」照样成立。"
                          + "但那三天正是旧话已经回应过的，不能拿来当新的开口理由。"
                          + "【就是手跑过的那一幕】-SeedLife → -CareAge → -CareFlat，"
                          + "09-23 实测 4 次：2 次不说（其中 1 次理由是错的）、2 次说"),

        EvalCase(id: 40, name: "退场后·换了一件别的事",
                 days: [(7, .tired, "连着加班"),
                        (6, .tired, "还是在改"),
                        (5, .tired, "改到半夜"),
                        (2, .calm,  "项目交了，松了口气"),
                        (1, .sad,   "和室友大吵一架，现在一个屋檐下很别扭")],
                 recentlySaid: [retiredHardTimes],
                 expected: true,
                 rationale: "加班结束了，新的难受换了来源（工作 → 关系）。"
                          + "旧话接不住，而且它已经撤下了 —— 这是干净的再开口。"
                          + "【和 #34 的分界】#34 那条还挂着，问的是「要不要替换」；"
                          + "这里没有东西可替换，问的是「要不要重新开口」。"
                          + "两条场景几乎一样，差别只在 stillShowing —— "
                          + "并排跑就能看出模型认不认得出这个区别"),

        // ────────────── 孤点专项：强烈，但只有一天，而且不是最近那天 ──────────────
        //
        // 09-23 从 `-SeedDiaries` 实跑里抓到的真实一幕（3/3 都说了外婆）。
        // 窗口直接照搬那份数据的后半段,不是编的。

        EvalCase(id: 41, name: "强烈的单点事件在四天前，之后两天平淡且不相关",
                 days: [(7, .happy,   "睡到自然醒，和室友玩桌游笑到肚子疼"),
                        (6, .angry,   "快递放错驿站，白跑了两趟"),
                        (5, .anxious, "三杯咖啡赶报告，下午总算写顺了"),
                        (4, .sad,     "外婆住院很揪心，视频里她还笑着"),
                        (2, .calm,    "在家洗了一周的衣服，平平静静"),
                        (1, .tired,   "被组长点名，火锅缓了口气，又加班到十点")],
                 recentlySaid: [],
                 expected: true,
                 rationale: "**这条主要拿来抽文案,不是考 shouldShow。**"
                          + "【原本标的是 false,测完翻了】假设是:这份数据没有趋势(情绪乱跳、"
                          + "每件事当天就消化),关怀会退化成「挑出最惨的一天说一句」,"
                          + "而 §2.1 的铁律是趋势是天与天之间的事。"
                          + "**跑 20 次,17 次说了,8 条样本全部指向外婆** —— 而且有两条把外婆和工作"
                          + "串成了一件事(「外婆的事压在心里,还照常上班」),那是综合不是挑单点。"
                          + "模型的读法站得住:**住院不会因为日记里没再提就结束**,"
                          + "之后两天的洗衣服和加班不构成「这事翻篇了」。所以翻成 true。"
                          + "【但违和感是真的,它在文案上】09-23 真机实跑时的直觉是「怎么讲四天前的事」。"
                          + "测完才看清那不是 shouldShow 的问题,是两点文案问题:"
                          + "① **「外婆住院那几天」—— 外婆只出现在一天**(之后是洗衣服、被组长点名),"
                          + "   一天说成几天是**事实说错**,比时间词过期严重。"
                          + "   prompt 里禁了「今天」「刚刚」这类**会过期的时间词**,但没禁这类**范围词**。"
                          + "② 8 条里 5 条只说外婆、不提之后那两天 —— 用户会觉得「她没在跟上我」。"
                          + "   若 1、2 天前是空白,说 4 天前完全合理;正因为之后写了两天,跳过才刺眼。"
                          + "【抽查口径】这条判 true 是对的,要看的是**它说的那句**:"
                          + "出现「那几天」「这几天」这类把单日说成多日的范围词,就算没过"),
        ]
}
