...
"""
    ...
    private static let carePrompt = persona + """
        ...
        有关心挂着时，时间线里每天会带 isNew 字段：
        isNew: true 只说明这是当前关心之后才出现的信息，不说明应该替换。
        是否替换，要看这些新信息和之前的内容相比，是否构成下面的实质变化；
        isNew: false 的内容（包括「针对」里那几天）已经被回应过，是判断「变了没有」的参照。
        ...
        主要困扰或情绪主题发生变化，新主题已经成为当前状态的重点，当前消息接不住它；

        安全风险高于当前关心的安全等级。安全等级升高时应立即替换。

        新写了一篇日记、表达方式不同、轻微起伏、原有状态继续延续，都不属于实质变化。
