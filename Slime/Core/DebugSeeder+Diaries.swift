...
///
/// 每天对应一个要验的场景：
/// - 今天 4 篇、没蛋 —— 按母鸡孵，再删一篇看蛋是否作废、能否重按；
///   头一篇是**刻意写长的**，卡片装不下，用来验证卡片内竖滚和底部渐隐
...
extension DebugSeeder {
    ...
    private static let diaryLife: [SeedDay] = [
        SeedDay(daysAgo: 0, egg: nil, entries: [
            // 唯一一篇超出卡片高度的。别改短 —— 它是滚动和底部渐隐的验证用例
            SeedEntry(hour: 0, emotion: .anxious, text: """
                昨晚又是两点才睡着，五点半就醒了，躺着翻来覆去，干脆爬起来把这阵子的事从头到尾想了一遍。

                项目从六月拖到现在，需求改了四版，每次都说这是最后一版，然后过两天又有新的。我知道不该跟这个较劲，可每次推倒重来的时候还是会想，前面那两个礼拜到底算什么。上周跟小林吃饭，她说她也一样，公司不一样，事情几乎一模一样，我们俩在火锅店里笑了半天，笑完谁也没说出个解法。

                今天早上倒是想通一件事：我好像一直在等某个节点，等这版交了、等这个季度过了，就能松一口气。可去年这个时候我也是这么想的，前年大概也是。也许根本没有那个节点，只有一天接着一天。

                天亮了，楼下早餐店已经开门了，我去买个面包。
                """),
